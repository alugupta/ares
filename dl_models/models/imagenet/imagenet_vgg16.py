...
class imagenetVGG16(ModelBase):
  def __init__(self):
    ...
    self.num_train_blocks = 5000
    self.num_val_blocks   = 178
    self.num_epochs       = 1
    ...
    preprocessing_dir = imagenetVGG16_params['preprocessing_dir']

    print "Preprocessing dir = ", preprocessing_dir

    self.weights_file_name = preprocessing_dir + "imagenet_vgg16_pretrained_th_weights"
    #self.weights_file_name = preprocessing_dir + "imagenet_vgg16_pretrained_weights"
  ...
  def build_model(self,):
    # Recreate the Keras VGG16 pre-built model here:
  ...
  def compile_model(self, loss='categorical_crossentropy', optimizer='sgd', metrics=None):
    ...
    if optimizer is 'sgd':
      print "Using VGG specific  optimizer SGD"

      learning_rate = imagenetVGG16_params['learning_rate']
