...
class tidigitsLSTM(ModelBase):
  ...
  def build_model(self,):
    ...
    model.add(LSTM(100, return_sequences = True, input_shape = (self.timesteps, self.features)))
  ...
  def load_dataset(self, ):
    try:
      ...
      print "Please set the correct path to the pre-processed dataset in 'ares/dl_models/models/tidigits/tidigts_lstm.py'"
  ...
  def compile_model(self, loss='categorical_crossentropy', optimizer='sgd', metrics=None):
    self.model.compile(loss=loss,
                         optimizer=optimizers.SGD(lr = 0.005, momentum = 0.9, decay = 0, nesterov=False),
                         metrics=['accuracy'])
